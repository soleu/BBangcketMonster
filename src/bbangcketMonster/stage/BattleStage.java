...
import bbangcketMonster.Character;
import bbangcketMonster.GameManager;
import bbangcketMonster.Guild;
import bbangcketMonster.Ork_Rival_Unit;
import bbangcketMonster.RivalUnit;
import bbangcketMonster.Zombie_Rival_Unit;
...
public class BattleStage implements Stage {
    ...
    Map<Integer, RivalUnit> MonsterList = new HashMap<Integer, RivalUnit>();
    Vector<RivalUnit> currentMonster;
        ...
        while (true) {
            ...
            if (turn == false) {
                ...
                for (int i = 0; i < currentGuild.size(); i++) {
                    Character character = currentGuild.get(i);
                    if (character.isDead == true)
                        continue;
                    System.out.println(character.name + "의 턴");
                    ...
                    if (choice == 1) {
                        ...
                        character.attack(currentMonster.get(j));
                    } else if (choice == 2) {
                ...
                for (int i = 0; i < currentMonster.size(); i++) {
                    RivalUnit character = currentMonster.get(i);
                    if (character.isDead == true)
                        continue;
                    int j = 0;
                    ...
                    character.attack(currentGuild.get(j));
                }
                turn = false;
            }
        }
...
//				System.out.println(guild.guildMember.get(i).name);
                Character character = guild.guildMember.get(i);
                currentGuild.add(new Character(character));
            }
        }
    }

    void initMonster() {
        MonsterList.put(0, new Zombie_Rival_Unit());
        MonsterList.put(1, new Ork_Rival_Unit());
    }
